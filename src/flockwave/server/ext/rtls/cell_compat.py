...
that contains those helpers (the converged-sim ``cell.py``, SDK commit 7). The
server only needs three things from the cell model -- the device role, the
origin + anchor NED table, and NED->global(E7) -- so the fallback implements
